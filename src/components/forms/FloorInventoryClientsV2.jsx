...
export default function FloorInventoryClientsV2() {
  ...
  return (
    <div className="space-y-6">
            ...
            <div key={cIdx} className="rounded-2xl border-2 border-gray-200 p-4 bg-white">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
                <div>
                  ...
                  <select value={cliente.tipoCliente || 'colo'} onChange={(e) => updateFloorClientFieldV2(cIdx, 'tipoCliente', e.target.value)} className="w-full px-3 py-2 text-sm border-2 border-gray-200 rounded-xl bg-white focus:outline-none focus:border-primary">
              ...
              <div className="mb-3">
                ...
                <input value={cliente.placaEquipos || ''} onChange={(e) => updateFloorClientFieldV2(cIdx, 'placaEquipos', e.target.value)} placeholder="Ej: placa/identificadores" className="w-full px-3 py-2 text-sm border-2 border-gray-200 rounded-xl bg-white focus:outline-none focus:border-primary" />
              </div>
                ...
                <div className="space-y-2">
                  {(cliente.gabinetes || []).map((gab, gIdx) => (
                    <div key={gIdx} className="grid grid-cols-6 gap-2 items-end">
                      <Input label={gIdx === 0 ? 'Gabinete' : ''} value={gab.gabinete || ''} onChange={(e) => updateCabinetFieldV2(cIdx, gIdx, 'gabinete', e.target.value)} placeholder="Gab-01" className="mb-0" />
                      ...
                      <Input label={gIdx === 0 ? 'Foto #' : ''} value={gab.fotoRef || ''} onChange={(e) => updateCabinetFieldV2(cIdx, gIdx, 'fotoRef', e.target.value)} placeholder="3" className="mb-0" />
                      <button type="button" onClick={() => removeCabinetV2(cIdx, gIdx)} className="w-9 h-9 rounded-xl border-2 border-gray-200 text-gray-600 bg-white hover:border-red-300 hover:text-red-600 active:scale-95 flex items-center justify-center">
                        <Trash2 size={14} />
                      </button>
                    </div>
                  ))}
      ...
      {/* Foto plano de planta */}
      <div className="bg-white rounded-2xl border border-gray-200 p-4">
        <label className="block text-sm font-bold text-gray-900 mb-2">Foto de plano de planta y equipos</label>
        <PhotoUpload type="after" photo={fotos.fotoPlanoPlanta || null} onCapture={(data) => updateEquipmentV2Field('fotos', 'fotoPlanoPlanta', data)} onRemove={() => updateEquipmentV2Field('fotos', 'fotoPlanoPlanta', null)} formCode="equipment-v2" assetType="equipmentV2:fotoPlanoPlanta" />
      </div>
    </div>
  )
}
