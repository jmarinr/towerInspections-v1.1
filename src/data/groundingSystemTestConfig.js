export const groundingSystemTestConfig = {
  id: 'grounding-system-test',
  title: 'Prueba de puesta a tierra',
  subtitle: 'Medición de resistencia del sistema de puesta a tierra y evidencia.',
  sections: [
    {
      id: 'datos',
      title: 'Datos del sitio',
      description: 'Identificación del sitio y datos de la visita.',
      fields: [
        { id: 'proveedor', label: 'Proveedor', type: 'text', required: true, placeholder: 'Ej: PTI / Contratista', defaultValue: 'OFG PANAMA, S.A.' },
        { id: 'tipoVisita', label: 'Tipo de visita', type: 'select', required: true, defaultValue: 'mantenimiento', options: [
          { value: '', label: 'Seleccione...' },
          { value: 'inspeccion', label: 'Inspección' },
          { value: 'mantenimiento', label: 'Mantenimiento' },
          { value: 'prueba', label: 'Prueba' },
          { value: 'otro', label: 'Otro' },
        ]},
        { id: 'idSitio', label: 'ID del sitio', type: 'text', placeholder: 'Ej: CR-SJ-001' },
        { id: 'nombreSitio', label: 'Nombre del sitio', type: 'text', placeholder: 'Ej: Torre Escazú' },
        { id: 'direccion', label: 'Dirección', type: 'text', placeholder: 'Dirección del sitio' },
        { id: 'tipoSitio', label: 'Tipo de sitio', type: 'select', options: [
          { value: '', label: 'Seleccione...' },
          { value: 'torre', label: 'Torre' },
          { value: 'rooftop', label: 'Rooftop' },
          { value: 'indoor', label: 'Indoor' },
          { value: 'otro', label: 'Otro' },
        ]},
        { id: 'tipoEstructura', label: 'Tipo de estructura', type: 'select', options: [
          { value: '', label: 'Seleccione...' },
          { value: 'autosoportada', label: 'Autosoportada' },
          { value: 'arriostrada', label: 'Arriostrada' },
          { value: 'monopolo', label: 'Monopolo' },
          { value: 'otro', label: 'Otro' },
        ]},
        { id: 'alturaMts', label: 'Altura (mts)', type: 'number', placeholder: '0' },
      ],
    },
    {
      id: 'condiciones',
      title: 'Condiciones del terreno',
      description: 'Contexto de la medición según condiciones ambientales.',
      fields: [
        { id: 'estadoTerreno', label: 'Estado del terreno', type: 'toggle', options: [
          { value: 'humedo', label: 'Húmedo' },
          { value: 'seco', label: 'Seco' },
          { value: 'mixto', label: 'Mixto' },
        ]},
        { id: 'tipoTerreno', label: 'Tipo de terreno', type: 'select', options: [
          { value: '', label: 'Seleccione...' },
          { value: 'arcilloso', label: 'Arcilloso' },
          { value: 'arenoso', label: 'Arenoso' },
          { value: 'rocoso', label: 'Rocoso' },
          { value: 'mixto', label: 'Mixto' },
          { value: 'otro', label: 'Otro' },
        ]},
        { id: 'ultimoDiaLluvia', label: 'Último día de lluvia', type: 'select', options: [
          { value: '', label: 'Seleccione...' },
          { value: 'hoy', label: 'Hoy' },
          { value: 'ayer', label: 'Ayer' },
          { value: '2dias', label: 'Hace 2 días' },
          { value: '3dias', label: 'Hace 3+ días' },
          { value: 'na', label: 'No aplica / Desconocido' },
        ]},
        { id: 'hora', label: 'Hora de medición', type: 'time' },
        { id: 'notaMetodo', label: 'Nota (método)', type: 'textarea', placeholder: 'Si es rooftop y no se pueden colocar picas, describa el método alterno.' },
      ],
    },
    {
      id: 'equipo',
      title: 'Equipo de medición',
      description: 'Datos del equipo utilizado.',
      fields: [
        { id: 'equipoMarca', label: 'Marca', type: 'text', placeholder: 'Ej: Fluke' },
        { id: 'equipoModelo', label: 'Modelo', type: 'text', placeholder: 'Ej: 1625-2' },
        { id: 'equipoSerial', label: 'Serial', type: 'text', placeholder: 'Número de serie' },
        { id: 'equipoCalibracion', label: 'Fecha de calibración', type: 'date' },
        { id: 'fotoConexionTelurometro', label: 'Foto - Conexión telurómetro al sistema de tierra', type: 'photo' },
        { id: 'fotoElectrodoCorriente', label: 'Foto - Conexión del electrodo de corriente', type: 'photo' },
      ],
    },
    {
      id: 'medicion',
      title: 'Medición de resistencia',
      description: 'Registre los valores medidos (Ohms) y evidencia fotográfica por punto.',
      fields: [
        { id: 'distanciaElectrodoCorriente', label: 'Distancia elect. corriente (mts)', type: 'number', placeholder: '0' },
        { id: 'rPataTorre',    label: 'Pata de la torre (Ω)',  type: 'number', placeholder: '0.00' },
        { id: 'fotoPataTorre', label: 'Medición 1 - Conexión electrodo potencial (Pata de la torre)', type: 'photo', dataSectionId: 'evidencia' },
        { id: 'rCerramiento',    label: 'Cerramiento (Ω)',  type: 'number', placeholder: '0.00' },
        { id: 'fotoCerramiento', label: 'Medición 2 - Conexión electrodo potencial (Cerramiento)', type: 'photo', dataSectionId: 'evidencia' },
        { id: 'rPorton',    label: 'Portón (Ω)',  type: 'number', placeholder: '0.00' },
        { id: 'fotoPorton', label: 'Medición 3 - Conexión electrodo potencial (Portón)', type: 'photo', dataSectionId: 'evidencia' },
        { id: 'rPararrayos',    label: 'Pararrayos (Ω)',  type: 'number', placeholder: '0.00' },
        { id: 'fotoPararrayos', label: 'Medición 4 - Conexión electrodo potencial (Pararrayos)', type: 'photo', dataSectionId: 'evidencia' },
        { id: 'rBarraSPT',    label: 'Barra SPT (Ω)',  type: 'number', placeholder: '0.00' },
        { id: 'fotoBarraSPT', label: 'Medición 5 - Conexión electrodo potencial (Barra de SPT)', type: 'photo', dataSectionId: 'evidencia' },
        { id: 'rEscalerilla1',    label: 'Escalerilla #1 (Ω)',  type: 'number', placeholder: '0.00' },
        { id: 'fotoEscalerilla1', label: 'Medición 6 - Conexión electrodo potencial (Escalerilla)', type: 'photo', dataSectionId: 'evidencia' },
        { id: 'rEscalerilla2',    label: 'Escalerilla #2 (Ω)',  type: 'number', placeholder: '0.00' },
        { id: 'fotoEscalerilla2', label: 'Medición 7 - Conexión electrodo potencial (Escalerilla)', type: 'photo', dataSectionId: 'evidencia' },
        { id: 'sumResistencias', label: 'Sumatoria (automática)', type: 'calculated' },
        { id: 'rg', label: 'Rg promedio (automático)', type: 'calculated' },
        { id: 'observaciones', label: 'Observaciones', type: 'textarea', placeholder: 'Notas relevantes de la medición.' },
        { id: 'fotoObservaciones', label: 'Foto - Observaciones', type: 'photo' },
      ],
    },
  ],
}
